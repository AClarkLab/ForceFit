...
public:
	...
	virtual void prepareMD(std::vector<GeometrySet> geoSets, std::vector<float> variables, std::vector<struct mdQuestion> questions) = 0;
	...
	const std::vector<float> & getVariables();
	void setVariables(const std::vector<float> & values);
